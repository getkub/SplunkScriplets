...
CONF_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'files', 'macros.conf')
SETTINGS_PATH = os.path.join(BASE_DIR, '.vscode', 'settings.json')
CONFIG_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'files', 'config.json')
CERT_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'files', 'splunk-8089.crt')
...
for stanza in conf_parser.sections():
    ...
    # For macros, we need to handle the definition and args fields specially
    if 'definition' in payload:
        payload['definition'] = payload['definition'].strip('"')  # Remove quotes if present
    if 'args' in payload:
        payload['args'] = payload['args'].strip('"')  # Remove quotes if present

    url = f"{baseUrl}/servicesNS/{owner}/{app}/data/macros/{name}"
    try:
    ...
    if args.action_flag == 'deleteOnly':
        ...
        if exists:
            ...
            if not args.dry_run:
                update_url = f"{baseUrl}/servicesNS/{owner}/{app}/data/macros/{name}"
            ...
            if not args.dry_run:
                ...
                create_url = f"{baseUrl}/servicesNS/{owner}/{app}/data/macros"
